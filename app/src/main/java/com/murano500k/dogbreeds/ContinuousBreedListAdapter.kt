...
class ContinuousBreedListAdapter(val breedsViewModel: ContinuousListBreedsViewModel) : RecyclerView.Adapter<ContinuousBreedListAdapter.DogBreedViewHolder>() {
    ...
    fun addDogBreedList(dogBreedList: List<DogBreed>) {
        ...
        val new = dogBreedList.size
        if(new > previous){
            items.addAll(dogBreedList.subList(previous, dogBreedList.lastIndex))
        }
        //items.addAll(dogBreedList)
        notifyItemInserted(dogBreedList.size-1)
